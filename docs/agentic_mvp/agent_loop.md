...
5. 为新回合选择 `coc-tools-agentic-mvp-1` 的默认五工具 profile，将它冻结进未完成回合；启动第一次执行尝试，记录 180 秒 deadline、8 次往返余额和一次结构修正余额。
6. 组装本轮初始 GM 上下文。
...
完整游戏记录不包含模型隐藏推理。Increment 1 的历史 profile 只暴露 `make_check`；Increment 3 的新回合默认同时暴露 `make_check`、`push_check`、`spend_luck`、`deal_damage` 和 `make_sanity_check`。完整注册表仍保留 `coc-tools-agentic-mvp-1` 已发布的全部验证器，profile 只选择本回合向模型公开的工具子集。
...
收到一个工具调用后，Harness 先检查工具是否属于本轮冻结 profile，并按 `(turn_id, tool_call_id)` 查找既有交互。相同 ID 和相同规范参数只复用已保存结果，不同参数直接返回协议错误；重放不进入新的机械生命周期。对尚未处理的新调用，所有工具统一按以下顺序处理：

1. **规范化**：实际工具解析 provider 原始参数，拒绝未知字段、错误类型和非法语法。
2. **领域 preflight 与冻结输入**：实际工具从角色卡和不可变机械历史读取权威数据，完成角色、能力、可见性、余额、补救链和表达式边界等全部可预先完成的校验，并返回执行所需的可信快照。此阶段不分配机械 ID/提交时间，也不调用 RNG。
3. **分配 ID 与时间**：只有 preflight 成功后，Harness 才分配 `mechanic_id` 和 `committed_at`；工具不能接收或伪造它们。
4. **RNG 与执行**：工具只使用冻结输入和 Harness 注入的随机源，生成机械结果及候选角色变化；结果必须通过该工具自己的严格校验。
5. **原子提交**：Harness 在一次完整 `session.json` 原子替换中同时写入 assistant tool-call、`ToolInteraction`、对应 tool result 消息、机械记录和角色数值变化。提交失败时这些候选内容都不生效。
6. **公开投影**：提交成功后，若机械为 `public`，实际工具从完整机械选择允许玩家看到的 `details`，Harness 构造统一的 `PublicMechanic(mechanic_id, kind, actor_id, details)` 并发布；隐藏机械不产生玩家投影。
7. **继续同一 GM**：将已经提交的完整结果及原 `tool_call_id` 加入同一 GM 对话。

规范化或领域 preflight 失败时，不分配机械 ID/时间、不掷骰、不改变数值；失败 `ToolInteraction` 与可配对协议消息仍按既有错误提交契约保存。GM 可以在剩余预算内修正调用或根据当前虚构完成回合。Harness 不把工具拒绝改写成剧情结果，也不为 `make_check` 或其他工具增加名称特判。
...
孤注一掷通常跨两个玩家回合完成：第一次失败后，GM 在已提交叙事中说明机会；玩家在下一条输入中提出不同做法并明确接受风险。GM 负责理解这项玩家选择，随后调用 `push_check`，引用原失败检定并在结果产生前说明更严重的失败风险。Harness 只允许选中玩家调查员的公开基础失败检定进入 Push，并验证关联、提交时的资格快照、整条不可变补救链和一次性结算，不建设自然语言意图分类器。

GM 不自动替玩家孤注一掷。孤注一掷结果不是对原骰点的覆盖，两次机械记录都保留。

### `spend_luck`

只有玩家在当前输入中明确选择花费幸运时，GM 才调用 `spend_luck`。Harness 只允许选中玩家调查员的公开基础失败检定进入 Luck，并从原检定读取 `roll` 和声明难度的 `target`，要求 `points = roll - target`；不足、超量、目标为 0、fumble、pushed 结果或已进入其他补救链都拒绝。成功只把有效结果提高到原检定声明的难度，不能制造 `critical_success`。GM 不能在叙事中代替玩家补做该选择。
...
COC 工具采用“先持久化，后投影和报告”：

- 成功工具调用的随机数与数值变化立即成为机械权威。
- 工具成功响应、CLI 的统一 `PublicMechanic` 投影和送给 GM 的 tool result 都源自同一持久化记录；公开字段由实际工具选择，Harness 不按 `kind` 猜测结构。
...
- 沿用 `IncompleteTurn.model_profile`、`attempt_limits`、Prompt 修订、工具 schema 版本和启用工具列表；恢复使用该回合自己的冻结 profile，不要求它等于 Harness 的新回合默认 profile。Harness 必须逐项验证冻结工具子集仍由 `coc-tools-agentic-mvp-1` 完整注册表支持；缺少实现、版本不可用或 profile 损坏时明确报错，不静默切换配置。
...
- 新工具调用严格遵循“规范化、领域 preflight、分配 ID/时间、RNG/执行、原子提交、公开投影”；Harness 不按工具名或 `kind` 特判领域规则和玩家字段。
