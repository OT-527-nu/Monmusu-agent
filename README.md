...
安装项目后也可运行 `monmusu-agent-agentic`。该入口从外部环境读取 `DEEPSEEK_API_KEY`，可用 `MONMUSU_DEEPSEEK_MODEL_ID` 和 `MONMUSU_DEEPSEEK_THINKING` 显式覆盖默认的 `deepseek-v4-flash`、non-thinking 配置；它把会话写入 `var/agentic_sessions/`，展示开场并在同一 GM 会话中持续接收玩家行动，直到本局收束或发生技术中断。Increment 1 会在请求前拒绝 thinking 模式；默认 `monmusu-agent` 入口和旧存档不受影响。
