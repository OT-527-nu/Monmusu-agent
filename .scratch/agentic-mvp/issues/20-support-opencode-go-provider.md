...
- [ ] `DeepSeekGameMasterModel` 接受 provider 对应的 base_url，并在 opencode-go profile 下构造 OpenAI SDK 请求：`base_url=https://opencode.ai/zen/go/v1`、`model=deepseek-v4-flash`、`stream=false`、`response_format=json_object`、`max_tokens` 与现有 profile 一致。
- [ ] opencode-go 首版只允许 `thinking=false`。基于 dsh/pi-ai catalog 中 opencode-go `deepseek-v4-flash` 的 `thinkingFormat: deepseek` 事实，请求继续发送 `extra_body={"thinking":{"type":"disabled"}}`；设置 `thinking=true` 时在模型调用前返回稳定配置错误。
- [ ] opencode-go 的 assistant 回放消息必须携带 `reasoning_content` 字段：provider 返回 `null` 或缺失时，按 pi-ai 实现补空字符串后再回传，不把 reasoning 正文写入玩家记录、事实账本、普通日志或公开证据。该处理只适用于 opencode-go；DeepSeek 官方路径保持现有行为。
- [ ] mocked SDK 确定性测试覆盖 opencode-go 请求 URL、`model`、tools、JSON Object、`thinking: disabled`、`reasoning_content` 回放、usage 脱敏、鉴权/网络/限流错误映射，以及 key 不进入任何投影。
- [ ] opencode-go profile 的新回合默认只暴露 `deepseek-v4-flash`；未知或非 DeepSeek 模型在 Harness 调用前被拒绝，不建设模型能力表或自动选型。恢复继续使用冻结 profile 的 provider、model_id 与 base_url。
- [ ] 一次真实 opencode-go 契约由项目所有者提供 key 并显式启用时运行：direct final 与一次匹配 `tool_call_id` 的 `make_check` 往返都非跳过完成；无 key 时明确 skip，不得视为通过。
- [ ] 真实契约按现有评估格式记录 provider、model、thinking、stream、工具 schema、prompt/profile、timeout、usage、latency、脱敏请求投影和硬门结论；不保存 key、鉴权头、reasoning 正文、隐藏事实或原始 provider envelope。
- [ ] live 证据写入 `docs/agentic_mvp/evidence/`，与 DeepSeek 官方证据分开；本票只证明 opencode-go 的 SDK/协议可用，不评分 GM 质量、不进入模型评估矩阵、不宣称 opencode-go 成为默认 provider。
- [ ] 现有 DeepSeek 官方确定性测试、恢复测试和静态门禁保持通过；默认 provider 仍为 `deepseek`，不增加运行时 fallback 或自动路由。
