...
- 2026-08-18：实现后双轴审查发现：合法 JSON 可用对象型 `schema_version` 让完整 loader 抛出裸 `TypeError`，原目录层只捕获 `AgenticSessionError`，仍可能中止全部扫描。新增该失败反例后，将每个候选目录视为独立的不可信边界，单条普通异常统一收敛为固定 `SessionCatalogIssue`；`KeyboardInterrupt` 与 `SystemExit` 不在捕获范围。测试现同时断言有效与损坏 `session.json` 字节不变；修复后重新通过 31 个 session 测试、23 个 CLI 测试和全量 334 个确定性测试。
