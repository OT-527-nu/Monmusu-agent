...
- 验证：`PYTHONPATH=src .venv/bin/python -m unittest tests.test_agentic_harness tests.test_agentic_session`（59 passed）；`PYTHONPATH=src .venv/bin/python -m unittest discover -s tests`（168 passed）；targeted mypy、改动文件 Ruff 基础规则和 `git diff --check` 均通过。验证解释器为项目 `.venv` 的 Python 3.12.3。
